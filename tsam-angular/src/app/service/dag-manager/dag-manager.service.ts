    ...
    const modify = (data, pid = 0, level = 0) =>
      data
        ...
        .forEach((e) => {
          ...
          if (result[level - 1] && result[level]) {
            let sortedResults: Array<T> = []
            result[level - 1].forEach((i: T) => {
              let childrenArray = result[level].filter((ch: T) =>
              ...
              sortedResults =  sortedResults.filter((v,i) => sortedResults.findIndex(item => item.stepId == v.stepId) === i)
            })
